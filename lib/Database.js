...
class Database {
    ...
    getRoles() {
        return new Promise((resolve, reject) => {
            const connection = this.connect();
            // Now that departments can be deleted, departments.name can be null
            // If departments.name can be null, we want to display 'N/A' instead
            // COALESCE() allows us to do this
            const sql = `SELECT roles.title, roles.salary , COALESCE(departments.name, 'N/A') AS department_name
            FROM roles
    ...
    async addRole(title, salary, department) {
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.query(sql, params, (err, result) => {
                ...
                resolve(console.log(`
                The ${title} role has been added to the ${department} department! 
                `));
            });
    ...
    getEmployeeId(fullName) {
        return new Promise((resolve, reject) => {
            // Send back 'null' for the case in which we are trying to grab a managerId of an employee that does not have a manager. Or if the employee's manager is 'none'. 
            if (!fullName) {
                resolve(null);
            }
            const connection = this.connect();
    ...
    async addEmployee(firstName, lastName, role, manager) {
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.query(sql, params, (err, result) => {
                ...
                resolve(console.log(`
                ${firstName} ${lastName} has been added to the company db!
                `));
            });
            connection.end();
        })
    ...
    async updateEmployeeRole(employee, newRole) {
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.query(sql, params, (err, result) => {
                ...
                resolve(console.log(`
                ${employee}'s role has been changed to ${newRole}!
                `));
            });
            connection.end();
        });
    ...
    async updateEmployeeManager(employee, newManager) {
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.query(sql, params, (err, result) => {
                ...
                // Exit promise
                resolve(console.log(`
                ${employee}'s manager has been changed to ${newManager}!
                `));
            });
            connection.end();
        });
    ...
    // This function will get all the employees in a department, and SUM the values in the salary column given the name of the department
    async getDepartmentBudget(department) {
        // Get the departmentId from the name
        const departmentId = await this.getDepartmentId(department);
        return new Promise((resolve, reject) => {
            const connection = this.connect();
            // Select the SUM from the roles.salary column from the table that results from combining the employees table with the roles table
            // Additionally, only grab rows when the roles.department_id = departmentId of our desired department
            const sql = `SELECT SUM(roles.salary) AS department_budget
            FROM employees
            ...
            connection.query(sql, params, (err, rows) => {
                ...
                // Grab the value of the department_budget column, only one row so grab first item in returned map array. 
                const budget = rows.map(obj => obj.department_budget)[0];
                // Return a message that shows the depart name and it's budget.
                resolve(`
                The total utilized budget of the ${department} department is: $ ${budget}
                `);
            });
            connection.end();
        });
