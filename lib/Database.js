...
class Database {
    ...
    return new Promise((resolve, reject) => {
        const connection = this.connect();
        const sql = `SELECT employees.id, CONCAT_WS(' ', employees.first_name, employees.last_name) AS name, roles.title AS title, departments.name AS department, roles.salary AS salary
        FROM employees
